...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) 
    {
        ...
        CGRect pickerRect = CGRectMake(0.0, 0.0, frame.size.width, frame.size.height);
        
        // backgound
        UIImageView *bacground = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"pickerBackground.png"]];
        [bacground setFrame:pickerRect];
        [self addSubview:bacground];
        
        // content
        contentView = [[UIScrollView alloc] initWithFrame:pickerRect];
	...
	for (int index = firstNeededViewIndex; index <= lastNeededViewIndex; index++) 
	{
        if (![self isDisplayingViewForIndex:index]) 
		{
			...
			if (label == nil)
            {
                ...
                label.textColor = RGBACOLOR(0.0, 0.0, 0.0, 0.75);//[UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.75];//
            }
